...
const Plots = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'Доступен': return 'text-green-600 bg-green-100';
      case 'Продан': return 'text-red-600 bg-red-100';
      case 'Бронь': return 'text-yellow-600 bg-yellow-100';
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {plots.map((plot) => (
            <div key={plot.id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
              <div className="p-6">
                ...
                >
                  {plot.status === 'Доступен' ? 'Забронировать' : 'Недоступен'}
                </button>
              </div>
            </div>
          ))}
        ...
        <div className="mt-12 bg-white rounded-lg shadow-md p-8">
          ...
          <div className="bg-gray-100 rounded-lg p-8 text-center">
            <Icon name="Map" size={64} className="mx-auto text-gray-400 mb-4" />
